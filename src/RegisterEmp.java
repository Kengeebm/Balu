...
public class RegisterEmp extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost/naveen", "root", "root");
			// Preparing query
			// PreparedStateme nt pst1= con.prepareStatement("select emp_id from
			// emp_reg where emp_id=?");
			PreparedStatement pst2 = con
					.prepareStatement("insert into emp_table(FullName,DOJ,DOB,Department,Designation) values(?,?,?,?,?)");
			// pst1.setString(1, v2);
